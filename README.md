...
1. A Native application in your tenant you can use for this sample. The aplication needs to have a connection configured for it, and a user account in that connection that you can use
2. An API in your tenant, with an audience of 'reactnative:additionaltokens'
...
```bash
cd auth0-react-native-sample-additionaltokens
yarn install
```
...
Open the `android/app/src/main/AndroidManifest.xml` file and locate the Intent Filter declaration. You must update the `android:host` and 'android:scheme' properties to use your Auth0 Domain from the step above.
